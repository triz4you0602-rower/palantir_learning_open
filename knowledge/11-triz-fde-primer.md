...
- **온톨로지(Ontology)** — 현실을 **개체(Object Type)·관계(Link Type)·행동(Action Type)** 으로 모델링한다. 셋 다 팔란티어 Foundry의 공식 용어이며 각각 전용 docs 페이지가 있다 — 1차 인용은 [03](03-ontology.md) 출처 참조. 핵심은 마지막 것: 화면에서 버튼을 누르면 현실이 바뀌어야 한다.
  - ⚠️ **삼분 구조는 이 저장소의 단순화다.** 공식 docs(`ontology/core-concepts`)는 **속성(property)** 도 네 번째 스키마 요소로 정의한다. 또 원문 표기는 소문자(*object type*)이며 대문자 표기는 [03](03-ontology.md)을 따르는 저장소 관례다.
